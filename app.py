...
if df is None:
    st.error("Failed to load data. Please check if the data file exists and is valid.")
    st.stop()

# Sidebar title and filters
with st.sidebar:
    # Add title with mask icon (more appropriate for air pollution)
    st.markdown("<h1 style='text-align: center;'>🌫️ Invisible Pollution</h1>", unsafe_allow_html=True)
    st.markdown("---")
    st.header("Filters")
    ...
    selected_locations = st.multiselect("Select locations", all_locations, default=all_locations[:5])
    
    # Parameter filter
    st.subheader("Parameter")
    all_parameters = sorted(get_unique_values(df, 'parameter'))
    selected_parameter = st.selectbox("Select parameter", all_parameters, index=all_parameters.index('pm25') if 'pm25' in all_parameters else 0)
    
    # For correlation analysis
    st.subheader("Correlation Analysis")
    if selected_parameter:
        other_parameters = [p for p in all_parameters if p != selected_parameter]
        correlation_parameter = st.selectbox("Compare with parameter", other_parameters, index=0 if other_parameters else None)
...
parameter_filtered_df = filter_data_by_parameter(filtered_df, selected_parameter)
...
tab1, tab2, tab3, tab4 = st.tabs(["Trends", "Comparison", "Correlation", "Map"])

with tab1:
    st.subheader(f"{selected_parameter.upper()} Trends Over Time")
    st.markdown(f"Displaying trends for {get_parameter_info(selected_parameter)['description']}")
    time_series_chart = create_time_series_chart(parameter_filtered_df, selected_parameter)
...
with tab3:
    ...
    if correlation_parameter:
        ...
        correlation_df = filter_data_by_parameter(filtered_df, [selected_parameter, correlation_parameter])
        ...
        if not correlation_df.empty:
            ...
            st.warning("Insufficient data for correlation analysis with selected parameters.")
    else:
...
with tab4:
    ...
    map_df = geo_df.groupby(['location', 'lat', 'lon'])['value'].mean().reset_index()
...
# Footer
st.markdown("---")
st.markdown("Data source: Air Quality Monitoring Stations")
